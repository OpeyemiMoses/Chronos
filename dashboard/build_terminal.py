      ...
      <div class="ghost-sidebar-footer">
        <div class="ghost-user-card" id="ghostSidebarUserCard">
          <div style="display: flex; align-items: center; justify-content: space-between;">
            ...
            <span id="ghostSidebarStatusDot" style="color: #F59E0B; display: flex; align-items: center;">
              <i data-lucide="activity" style="width:13px;height:13px;stroke:#F59E0B;stroke-width:2;"></i>
            </span>
          </div>
            ...
            <div>
              ...
              <div class="stepper-vertical">
                ...
                <div class="stepper-item">
                  <div class="stepper-dot"><i data-lucide="check" style="width:11px;height:11px;stroke:currentColor;stroke-width:2.5;"></i></div>
                  <div>
        ...
        <div class="arena-grid-split">
          ...
          <div class="arena-card" style="display: flex; flex-direction: column; gap: 0.85rem; position: relative; overflow: hidden;">
            ...
            <div id="autoPilotControlPanel" style="display: flex; flex-direction: column; gap: 0.55rem;">
              ...
              <!-- Stress Test Engine Panel -->
              <div id="stressTestPanelContainer" style="margin-top: 0.1rem;"></div>
            </div>
          ...
          <div class="settings-card">
            ...
            <div id="bitgetInputsContainer" style="display: flex; flex-direction: column; gap: 1rem; transition: all 0.25s ease;">
              ...
              <div style="display: flex; gap: 0.65rem; flex-wrap: wrap;">
                <button id="btnSaveBitget" class="btn-execute-big" style="flex: 1; padding: 0.65rem; opacity: 0.45; cursor: not-allowed; pointer-events: none;" onclick="saveBitgetSettings()" disabled>
                ...
                <button id="btnClearBitget" class="preset-chip" style="padding: 0.65rem 1rem; font-weight: 700; color: var(--color-red); display: none;" onclick="clearBitgetCredentials()">
            ...
            <div id="liveBitgetStatusGroup" style="display: none; margin-top: 0.5rem;">
              <div style="display: flex; flex-direction: column; gap: 0.75rem; font-size: 0.82rem; color: #4B5563; font-family: var(--font-terminal);">
                <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(0,0,0,0.06); padding-bottom: 0.4rem;">
                  <span>Active Bitget Key:</span>
                  <strong style="color: #000;" id="liveBitgetKeyPreview">No Key Connected</strong>
                </div>
                <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(0,0,0,0.06); padding-bottom: 0.4rem;">
                  <span>Live Available Equity:</span>
                  <strong style="color: var(--color-green);" id="liveBitgetBalanceValue">— USDT</strong>
                </div>
                <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(0,0,0,0.06); padding-bottom: 0.4rem;">
                  <span>Account Margin Mode:</span>
                  <strong style="color: #000;" id="liveBitgetMarginMode">UTA Cross Margin</strong>
                </div>
                <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(0,0,0,0.06); padding-bottom: 0.4rem;">
                  <span>Gateway Status:</span>
                  <strong style="color: var(--color-amber);" id="liveBitgetGatewayStatus">Awaiting Connection</strong>
                </div>
              </div>
              <button class="btn-execute-big" style="width: 100%; margin-top: 1rem; padding: 0.6rem;" onclick="refreshBitgetAccount()">
                <span>Sync Live Balance from Bitget</span>
              </button>
            </div>
          </div>
          ...
          <div class="settings-card" style="grid-column: 1 / -1;">
            ...
            <div style="background: var(--color-canvas-subtle); border-radius: 8px; padding: 1rem 1.25rem; margin-top: 1.25rem; border: 1px solid rgba(0,0,0,0.06);">
              ...
              <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 0.75rem; font-size: 0.76rem; color: #4B5563;">
                <div style="display: flex; align-items: center; gap: 0.45rem;"><i data-lucide="check" style="width:13px;height:13px;stroke:#10B981;stroke-width:2.5;flex-shrink:0;"></i> <span>Statistical Dislocation (|Drift| ≥ 2.0%)</span></div>
                <div style="display: flex; align-items: center; gap: 0.45rem;"><i data-lucide="check" style="width:13px;height:13px;stroke:#10B981;stroke-width:2.5;flex-shrink:0;"></i> <span>Z-Score Divergence Threshold (|Z| ≥ 2.0σ)</span></div>
                <div style="display: flex; align-items: center; gap: 0.45rem;"><i data-lucide="check" style="width:13px;height:13px;stroke:#10B981;stroke-width:2.5;flex-shrink:0;"></i> <span>Within Max Concurrent Trades Quota</span></div>
                <div style="display: flex; align-items: center; gap: 0.45rem;"><i data-lucide="check" style="width:13px;height:13px;stroke:#10B981;stroke-width:2.5;flex-shrink:0;"></i> <span>No Duplicate Positions for Same Asset</span></div>
                <div style="display: flex; align-items: center; gap: 0.45rem;"><i data-lucide="check" style="width:13px;height:13px;stroke:#10B981;stroke-width:2.5;flex-shrink:0;"></i> <span>Sufficient Free Margin in Connected Vault</span></div>
                <div style="display: flex; align-items: center; gap: 0.45rem;"><i data-lucide="check" style="width:13px;height:13px;stroke:#10B981;stroke-width:2.5;flex-shrink:0;"></i> <span>Institutional NYSE/Nasdaq Market Closed</span></div>
              </div>
            </div>
  ...
  <div id="tradeReasoningModal" class="trade-reasoning-overlay" onclick="handleReasoningBackdropClick(event)">
    <div class="trade-reasoning-card" onclick="event.stopPropagation()">
      ...
      <div class="trade-reasoning-header">
        ...
        <button class="trade-reasoning-close" onclick="closeTradeReasoningModal()" aria-label="Close modal"><i data-lucide="x" style="width:16px;height:16px;stroke:currentColor;stroke-width:2;"></i></button>
      </div>
      ...
      <div class="reasoning-split-layout">
        ...
        <div class="reasoning-main-content">
          ...
          <div class="reasoning-tab-bar">
            ...
            <button type="button" class="reasoning-tab-btn" id="tabBtnStresstest" onclick="switchReasoningTab('stresstest')" style="position: relative;">
              <span>5. Stress Test</span>
              <span id="stressTabBadge" style="display: none; position: absolute; top: 3px; right: 3px; width: 7px; height: 7px; border-radius: 50%; background: #10B981;"></span>
            </button>
          </div>
          ...
          <div class="reasoning-tab-pane-container">
            ...
            <!-- Pane 5: Stress Test Report (always populated before the agent trades) -->
            <div class="reasoning-tab-pane" id="paneStresstest">
              <h3 class="pane-headline">Pre-Trade Stress Test — Agent Scenario Analysis</h3>
              <p class="pane-text" id="paneStressIntro" style="margin-bottom: 0.75rem;">The Chronos agent runs this 6-scenario stress test on every token before committing any capital. Results are stored in persistent memory so prior outcomes calibrate future entry thresholds.</p>
              <!-- Stress Test content injected dynamically by openTradeReasoningModal -->
              <div id="paneStressTestContent">
                <div style="background: #FAF8F5; border: 1px solid #EEE9DF; border-radius: 8px; padding: 0.75rem; font-size: 0.74rem; color: #71717A;">No stress test data found for this trade. The agent will run one before the next entry.</div>
              </div>
            </div>
          </div>
        </div>
    ...
    const ChronosWalletStore = {{
      ...
      ensureWalletInitialized(addr) {{
        ...
        if (!d) {{
          ...
          if (!Array.isArray(d.paperOpenPositions)) {{
            d.paperOpenPositions = Array.isArray(d.openPositions) ? d.openPositions.slice() : [];
            modified = true;
          }}
          if (!Array.isArray(d.paperTrades)) {{
            d.paperTrades = Array.isArray(d.trades) ? d.trades.slice() : [];
            modified = true;
          }}
          if (!Array.isArray(d.liveOpenPositions)) {{
            d.liveOpenPositions = [];
            modified = true;
          }}
          if (!Array.isArray(d.liveTrades)) {{
            d.liveTrades = [];
            modified = true;
          }}
          // Auto-sanitize: If apiKey is a 64-char hex string (accidental secret paste), clear it
          if (d.gateway && d.gateway.apiKey && d.gateway.apiKey.length >= 60 && !d.gateway.apiKey.startsWith("bg_")) {{
            d.gateway.apiKey = "";
            d.gateway.apiSecret = "";
            d.gateway.passphrase = "";
            d.gateway.mode = "paper";
            d.liveBalance = null;
            modified = true;
          }}
          if (modified) {{
 ...
 renderOverviewDynamic(ChronosWalletStore.getCurrentData());
      ...
      syncActiveView() {{
        ...
        if (isConnected) {{
          ...
          if (sDot) {{
            sDot.style.color = "#10B981";
            sDot.innerHTML = `<i data-lucide="wifi" style="width:13px;height:13px;stroke:#10B981;stroke-width:2;"></i>`;
            refreshIcons();
          }}
        }} else {{
          ...
          if (sDot) {{
            sDot.style.color = "#EF4444";
            sDot.innerHTML = `<i data-lucide="wifi-off" style="width:13px;height:13px;stroke:#EF4444;stroke-width:2;"></i>`;
            refreshIcons();
          }}
        }}

        // Balance displays — show '—' when disconnected, or when Live without connected real balance
        const isLive = typeof activeTradingEnv !== "undefined" && activeTradingEnv === "live";
        let balStr = "\u2014";
        if (isConnected && d) {{
          if (isLive) {{
            if (typeof d.liveBalance === "number") {{
              balStr = `$${{d.liveBalance.toLocaleString('en-US', {{minimumFractionDigits: 2, maximumFractionDigits: 2}})}}`;
            }} else {{
              balStr = "\u2014";
            }}
          }} else {{
            balStr = `$${{d.paperBalance.toLocaleString('en-US', {{minimumFractionDigits: 2, maximumFractionDigits: 2}})}}`;
          }}
        }}
        const setText = (id, val) => {{ const el = document.getElementById(id); if (el) el.textContent = val; }};
        setText("availBalanceDisplay", balStr);
        setText("arenaPaperBalanceDisplay", balStr);
        setText("settingsPaperBalanceDisplay", (isConnected && d && balStr !== "\u2014") ? balStr + " USDT" : "\u2014");
        setText("settingsWalletAddress", isConnected ? this.currentAddress : "--");
        setText("settingsAccountTypeLabel", isConnected ? (isLive ? "LIVE BITGET UTA ACCOUNT" : "CONNECTED WALLET") : "NOT CONNECTED");
        setText("ledgerWalletAddressLabel", isConnected ? (this.currentAddress.slice(0, 6) + "..." + this.currentAddress.slice(-4)) : "--");

        // Dynamic balance labels based on mode
        const availLabel = document.getElementById("availBalanceLabel");
        if (availLabel) availLabel.textContent = isLive ? "LIVE BITGET MARGIN" : "SIMULATION VAULT MARGIN";
        const arenaBalLabel = document.getElementById("arenaBalanceLabel");
        if (arenaBalLabel) arenaBalLabel.textContent = isLive ? "LIVE BITGET BALANCE" : "PAPER TRADING MARGIN";
        const settingsBalLabel = document.getElementById("settingsBalanceLabel");
        if (settingsBalLabel) settingsBalLabel.textContent = isLive ? "BITGET UTA REAL EQUITY" : "CURRENT VAULT BALANCE";
    ...
    function verifyStrategyClearance(symbol) {{
      ...
      // 5. Stress Test Gate — run 6-scenario analysis before every trade
      const stressResult = runStressTest(symbol, m.spot_price, side, requiredCollateral, "PRE_TRADE");
      if (!stressResult.error) {{
        storeStressTestResult(symbol, stressResult);
        // Refresh arena panel immediately
        if (typeof renderStressTestPanel === "function") renderStressTestPanel(symbol);
      }}
      ...
      return {{
        ...
        reason: `Strategy cleared: |Z| = ${{zScore.toFixed(2)}}σ ≥ ${{zThreshold.toFixed(2)}}σ, Drift = ${{m.drift_pct >= 0 ? '+' : ''}}${{m.drift_pct.toFixed(2)}}% vs Friday Anchor ($${{m.anchor_price.toFixed(2)}}). Stress Score: ${{stressResult && !stressResult.error ? stressResult.stress_score : 'N/A'}}/100 — CLEARED.`
      }};
    }}
    window.verifyStrategyClearance = verifyStrategyClearance;

    // =========================================================================
    // CHRONOS STRESS TEST ENGINE
    // Runs multi-scenario forward stress tests before every trade and after
    // every settlement. Results persisted in d.stressTestMemory per symbol.
    // =========================================================================

    /**
     * Run 6 deterministic forward scenarios for a given trade spec.
     * Returns a structured stress result with individual scenario P&L,
     * a composite stress score (0-100), break-even price, and worst-case loss.
     */
    function runStressTest(symbol, entryPrice, side, collateral, context) {{
      const m = markets[symbol];
      if (!m || !entryPrice || entryPrice <= 0) {{
        return {{ error: "Market data unavailable for stress test" }};
      }}

      const isShort = side === "SHORT" || side === "SELL_SHORT";
      const anchor = m.anchor_price;
      const gap = Math.abs(entryPrice - anchor);
      const feePct = 0.06; // taker fee

      function calcScenario(exitPrice, label, description) {{
        const rawDiff = isShort ? (entryPrice - exitPrice) : (exitPrice - entryPrice);
        const rawPct = (rawDiff / entryPrice) * 100;
        const netPct = parseFloat((rawPct - feePct).toFixed(2));
        const netUsd = parseFloat((collateral * (netPct / 100)).toFixed(2));
        return {{
          label,
          description,
          ...
          pnl_pct: netPct,
          pnl_usd: netUsd,
          verdict: netPct > 1.0 ? "STRONG_WIN" : netPct > 0 ? "MARGINAL_WIN" : netPct > -2.0 ? "MANAGEABLE_LOSS" : "SIGNIFICANT_LOSS"
        }};
      }}
      ...
      const breakEvenPrice = parseFloat((isShort
        ? entryPrice * (1 - beMovePct)
        : entryPrice * (1 + beMovePct)
      ).toFixed(2));

      // Load prior test memory for comparison note
      const d = ChronosWalletStore.getCurrentData();
      const history = (d && d.stressTestMemory && d.stressTestMemory[symbol]) || [];
      const lastTest = history.length > 0 ? history[history.length - 1] : null;

      let memoryNote = "No prior stress test recorded for this token.";
      if (lastTest) {{
        const zDiff = ((m.z_score || 0) - (lastTest.z_score || 0)).toFixed(2);
        const scoreDiff = stressScore - (lastTest.stress_score || 0);
      ...
      return {{
        ...
        context: context || "PRE_TRADE",
        symbol,
        side,
        entry_price: parseFloat(entryPrice.toFixed(2)),
        anchor_price: parseFloat(anchor.toFixed(2)),
        z_score: parseFloat((m.z_score || 0).toFixed(2)),
    ...
    /** Store a stress test result in wallet memory (max 10 per symbol) */
    function storeStressTestResult(symbol, result) {{
      const d = ChronosWalletStore.getCurrentData();
      if (!d) return;
      if (!d.stressTestMemory) d.stressTestMemory = {{}};
      if (!d.stressTestMemory[symbol]) d.stressTestMemory[symbol] = [];
      d.stressTestMemory[symbol].push(result);
      // Circular buffer: keep only last 10 per symbol
      if (d.stressTestMemory[symbol].length > 10) {{
        d.stressTestMemory[symbol] = d.stressTestMemory[symbol].slice(-10);
      }}
      ChronosWalletStore.setCurrentData(d);
    }}
    window.storeStressTestResult = storeStressTestResult;

    /** Get the most recent stored stress test for a symbol */
    function getLastStressTest(symbol) {{
      const d = ChronosWalletStore.getCurrentData();
      if (!d || !d.stressTestMemory || !d.stressTestMemory[symbol]) return null;
      const arr = d.stressTestMemory[symbol];
     ...
     * Shows the most recent stress test result for this symbol, or a prompt
     * to run one if none exists yet.
     */
    function renderStressTestPanel(symbol) {{
      ...
      function scenarioRow(sc, iconName, iconColor) {{
        const isProfit = sc.pnl_usd >= 0;
        const color = isProfit ? "#10B981" : "#EF4444";
        const sign = isProfit ? "+" : "";
        const ic = window.icon ? window.icon(iconName, {{ size: 13, color: iconColor }}) : `[${{iconName}}]`;
        return `
          <div style="display: grid; grid-template-columns: 1fr auto auto; gap: 0.5rem; align-items: center; padding: 0.28rem 0; border-bottom: 1px solid rgba(0,0,0,0.04);">
            <div style="display: flex; align-items: center; gap: 0.35rem;">
              ${{ic}}
              <div>
                <div style="font-size: 0.70rem; font-weight: 600; color: #18181B;">${{sc.label}}</div>
                <div style="font-size: 0.60rem; color: #A1A1AA;">${{sc.description}}</div>
              </div>
            </div>
            <div style="font-family: var(--font-terminal); font-size: 0.68rem; color: #52525B; text-align: right;">$${{sc.exit_price.toFixed(2)}}</div>
            <div style="font-family: var(--font-terminal); font-size: 0.70rem; font-weight: 700; color: ${{color}}; text-align: right; min-width: 70px;">${{sign}}$${{Math.abs(sc.pnl_usd).toFixed(2)}} (${{sign}}${{Math.abs(sc.pnl_pct).toFixed(2)}}%)</div>
          </div>
        `;
      }}

      const contextLabel = lastTest.context === "POST_AUDIT" ? "POST-SETTLEMENT RETROSPECTIVE" : "PRE-TRADE SCENARIO ANALYSIS";
      const timeStr = lastTest.timestamp ? new Date(lastTest.timestamp).toLocaleTimeString([], {{ hour: '2-digit', minute: '2-digit' }}) : "—";
      ...
      container.innerHTML = `
        <div style="background: #FFFFFF; border: 1px solid #EEE9DF; border-radius: 10px; padding: 0.9rem 1rem; box-shadow: 0 1px 4px rgba(0,0,0,0.04);">
          ...
          <div style="display: grid; grid-template-columns: 1fr auto auto; gap: 0.5rem; padding: 0.2rem 0; border-bottom: 1px solid #EEE9DF; margin-bottom: 0.25rem;">
            <div style="font-family: var(--font-terminal); font-size: 0.60rem; color: #A1A1AA; font-weight: 700;">SCENARIO</div>
            <div style="font-family: var(--font-terminal); font-size: 0.60rem; color: #A1A1AA; font-weight: 700; text-align: right;">EXIT $</div>
            <div style="font-family: var(--font-terminal); font-size: 0.60rem; color: #A1A1AA; font-weight: 700; text-align: right;">NET P&L</div>
          </div>

          <!-- Scenario Rows -->
          ${{scenarioRow(s.full_reversion, "target", "#10B981")}}
          ${{scenarioRow(s.partial_60, "trending-down", "#3B82F6")}}
          ${{scenarioRow(s.stall, "minus", "#A1A1AA")}}
          ${{scenarioRow(s.adverse_2pct, "alert-triangle", "#D97706")}}
          ${{scenarioRow(s.adverse_4pct, "alert-octagon", "#EF4444")}}
          ${{scenarioRow(s.tail_gap_8pct, "skull", "#7F1D1D")}}

          <!-- Summary Footer -->
          <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 0.55rem; padding-top: 0.45rem; border-top: 1px solid #EEE9DF;">
            <div style="font-family: var(--font-terminal); font-size: 0.64rem; color: #71717A;">
              Break-even: <strong style="color: #18181B;">$${{lastTest.break_even_price.toFixed(2)}}</strong>
              &nbsp;·&nbsp;
              Worst loss: <strong style="color: #EF4444;">-$${{Math.abs(lastTest.worst_case_loss_usd).toFixed(2)}}</strong>
            </div>
          ...
          <div style="margin-top: 0.5rem; padding: 0.4rem 0.6rem; background: rgba(99,102,241,0.05); border: 1px solid rgba(99,102,241,0.15); border-radius: 6px; font-size: 0.63rem; color: #6366F1; font-family: var(--font-terminal);">
            ${{window.icon ? window.icon('brain-circuit', {{ size: 12, color: '#6366F1' }}) : ''}} MEMORY: ${{lastTest.memory_note}}
          </div>
        </div>
      `;
      refreshIcons();
    ...
    function openTradeReasoningModal(tradeOrPosId) {{
      ...
      // ── Pane 5: Stress Test — populate from wallet memory ──
      const stressPaneContent = document.getElementById("paneStressTestContent");
      const stressTabBadge = document.getElementById("stressTabBadge");

      // Find the stress test that was run for this exact trade:
      // 1. Check if stored on the position/trade object directly
      // 2. Otherwise fall back to most recent memory for this symbol
      const tradeSymbol = meta.symbol || item.symbol;
      let stressData = item.stressTest || null;

      if (!stressData && tradeSymbol) {{
        stressData = getLastStressTest(tradeSymbol);
      }}
      ...
      if (stressPaneContent && stressData && stressData.scenarios) {{
        ...
        function modalScenarioRow(sc, iconName, iconColor) {{
          const isPos = sc.pnl_usd >= 0;
          const col = isPos ? "#10B981" : "#EF4444";
          const sign = isPos ? "+" : "";
          const ic = window.icon ? window.icon(iconName, {{ size: 13, color: iconColor }}) : `[${{iconName}}]`;
          return `
            <div style="display: grid; grid-template-columns: 1fr auto auto; gap: 0.5rem; align-items: center; padding: 0.3rem 0; border-bottom: 1px solid rgba(0,0,0,0.05);">
              <div style="display: flex; align-items: center; gap: 0.35rem;">
                ${{ic}}
                <div>
                  <div style="font-size: 0.72rem; font-weight: 600; color: #18181B;">${{sc.label}}</div>
                  <div style="font-size: 0.61rem; color: #A1A1AA;">${{sc.description}}</div>
                </div>
              </div>
              <div style="font-family: var(--font-terminal); font-size: 0.68rem; color: #52525B; text-align: right;">$${{sc.exit_price.toFixed(2)}}</div>
              <div style="font-family: var(--font-terminal); font-size: 0.72rem; font-weight: 700; color: ${{col}}; text-align: right; min-width: 75px;">${{sign}}$${{Math.abs(sc.pnl_usd).toFixed(2)}} (${{sign}}${{Math.abs(sc.pnl_pct).toFixed(2)}}%)</div>
            </div>`;
        }}

        stressPaneContent.innerHTML = `
          <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.7rem; padding: 0.6rem 0.85rem; background: rgba(99,102,241,0.05); border: 1px solid rgba(99,102,241,0.18); border-radius: 8px;">
            <div>
              <div style="font-family: var(--font-terminal); font-size: 0.63rem; color: #A1A1AA; font-weight: 700; margin-bottom: 0.1rem;">[STRESS TEST RESULT] · ${{stressData.context || 'PRE_TRADE'}} · ${{tradeSymbol}}</div>
              <div style="font-size: 0.72rem; color: #18181B; font-weight: 600;">${{stressData.side}} from $${{stressData.entry_price.toFixed(2)}} → Anchor $${{stressData.anchor_price.toFixed(2)}}</div>
              <div style="font-size: 0.63rem; color: #71717A; margin-top: 0.15rem;">Z=${{stressData.z_score.toFixed(2)}}σ · Drift=${{stressData.drift_pct >= 0 ? '+' : ''}}${{stressData.drift_pct.toFixed(2)}}% · Collateral: $${{stressData.collateral.toLocaleString()}} USDT</div>
            </div>
            <div style="text-align: right;">
              <div style="font-family: var(--font-terminal); font-size: 1.3rem; font-weight: 900; color: ${{scoreColor}}; line-height: 1;">${{stressData.stress_score}}<span style="font-size: 0.60rem;">/100</span></div>
              <div style="font-family: var(--font-terminal); font-size: 0.60rem; font-weight: 700; color: ${{scoreColor}};">${{scoreLabel}}</div>
              <div style="font-family: var(--font-terminal); font-size: 0.65rem; font-weight: 800; color: ${{recColor}}; margin-top: 0.05rem;">${{stressData.recommendation}}</div>
            </div>
          </div>

          <div style="display: grid; grid-template-columns: 1fr auto auto; gap: 0.5rem; padding: 0.2rem 0; border-bottom: 1px solid #EEE9DF; margin-bottom: 0.25rem;">
            <div style="font-family: var(--font-terminal); font-size: 0.60rem; color: #A1A1AA; font-weight: 700;">SCENARIO</div>
            <div style="font-family: var(--font-terminal); font-size: 0.60rem; color: #A1A1AA; font-weight: 700; text-align: right;">EXIT PRICE</div>
            <div style="font-family: var(--font-terminal); font-size: 0.60rem; color: #A1A1AA; font-weight: 700; text-align: right;">NET P&amp;L</div>
          </div>
          ${{modalScenarioRow(s.full_reversion, "target", "#10B981")}}
          ${{modalScenarioRow(s.partial_60, "trending-down", "#3B82F6")}}
          ${{modalScenarioRow(s.stall, "minus", "#A1A1AA")}}
          ${{modalScenarioRow(s.adverse_2pct, "alert-triangle", "#D97706")}}
          ${{modalScenarioRow(s.adverse_4pct, "alert-octagon", "#EF4444")}}
          ${{modalScenarioRow(s.tail_gap_8pct, "skull", "#7F1D1D")}}

          <div style="display: flex; justify-content: space-between; margin-top: 0.55rem; padding-top: 0.45rem; border-top: 1px solid #EEE9DF;">
            <div style="font-family: var(--font-terminal); font-size: 0.64rem; color: #71717A;">
              Break-even: <strong style="color: #18181B;">$${{stressData.break_even_price.toFixed(2)}}</strong>
              &nbsp;·&nbsp;
              Max loss: <strong style="color: #EF4444;">-$${{Math.abs(stressData.worst_case_loss_usd).toFixed(2)}}</strong>
            </div>
          ...
          <div style="margin-top: 0.55rem; padding: 0.4rem 0.6rem; background: rgba(99,102,241,0.05); border: 1px solid rgba(99,102,241,0.15); border-radius: 6px; font-size: 0.63rem; color: #6366F1; font-family: var(--font-terminal); display: flex; align-items: center; gap: 0.35rem;">
            ...
            AGENT MEMORY: ${{stressData.memory_note}}
          </div>

          <div style="margin-top: 0.55rem; padding: 0.4rem 0.65rem; background: rgba(${{stressData.recommendation === 'CLEARED' ? '16,185,129' : '239,68,68'}},0.06); border: 1px solid rgba(${{stressData.recommendation === 'CLEARED' ? '16,185,129' : '239,68,68'}},0.2); border-radius: 6px; font-size: 0.67rem; color: ${{stressData.recommendation === 'CLEARED' ? '#065F46' : '#7F1D1D'}}; font-family: var(--font-terminal); display: flex; align-items: center; gap: 0.35rem;">
            ${{window.icon ? window.icon(stressData.recommendation === 'CLEARED' ? 'check-circle' : 'x-circle', {{ size: 13, color: stressData.recommendation === 'CLEARED' ? '#10B981' : '#EF4444' }}) : ''}}
            ${{stressData.block_reason}}
          </div>
        `;

        refreshIcons();
        // Light up the green dot badge on the stress test tab
        if (stressTabBadge) stressTabBadge.style.display = "block";
      }} else if (stressPaneContent) {{
        stressPaneContent.innerHTML = `<div style="background: #FAF8F5; border: 1px solid #EEE9DF; border-radius: 8px; padding: 0.75rem; font-size: 0.74rem; color: #71717A;">No stress test data recorded for ${{tradeSymbol || 'this trade'}}. The agent will run a full 6-scenario analysis before the next entry.</div>`;
        if (stressTabBadge) stressTabBadge.style.display = "none";
      }}
        ...
        if (!isConnected) {{
          ...
          arenaSummary.innerHTML = `
            <div style="background: rgba(16,185,129,0.06); border: 1px solid rgba(16,185,129,0.22); border-radius: 8px; padding: 0.6rem 0.85rem; margin-top: 0.5rem; display: flex; align-items: center; justify-content: space-between; cursor: pointer;" onclick="switchView('trades')">
              <span style="font-family: var(--font-terminal); font-size: 0.70rem; font-weight: 700; color: #059669; display: inline-flex; align-items: center; gap: 0.25rem;"><i data-lucide="zap" style="width:11px;height:11px;stroke:#059669;stroke-width:2.5;"></i> ${{capLabel}}</span>
    ...
    function executeTradeOrder() {{
      ...
      const newPos = {{
        ...
        status: "ACTIVE",
        isLive: false,
        stressTest: getLastStressTest(m.symbol) || null
      }};
    ...
    function executeOpportunisticTrade(symbol) {{
      ...
      const newPos = {{
        ...
        status: "ACTIVE",
        isLive: false,
        stressTest: clearance.stressTest || getLastStressTest(m.symbol) || null
      }};
    ...
    function settleMondayMarketOpen() {{
      ...
      d.openPositions.forEach((pos, idx) => {{
        ...
        // ── REALISTIC SETTLEMENT: Based on actual live spot_price vs entry ──
        // The price ticker (Ornstein-Uhlenbeck) has been running all weekend,
        // pulling spot_price toward or away from anchor. We settle at exactly
        // that live price — no synthetic random win/loss override.
        const exitPrice = parseFloat((typeof m.spot_price === "number" ? m.spot_price : pos.entry_price).toFixed(2));
        ...
        if (isWin) {{
          totalWins++;
          const convergencePct = Math.abs(((exitPrice - m.anchor_price) / (pos.entry_price - m.anchor_price || 1)) * 100).toFixed(1);
          settlementActionNote = `Monday 08:30 EST institutional auction settled @ $${{exitPrice.toFixed(2)}} (Entry $${{pos.entry_price.toFixed(2)}}, Anchor $${{m.anchor_price.toFixed(2)}}). Mean-reversion captured.`;
          auditVerdict = "PROFITABLE_RESILIENCE_AUDIT";
          rootCause = `Monday open returned price toward fundamental anchor ($${{m.anchor_price.toFixed(2)}}). Spot at settlement: $${{exitPrice.toFixed(2)}}.`;
          adaptation = `Strategy parameters verified for ${{pos.symbol}}. Preserved 100% cash allocation until next Friday 16:00 EST.`;
        }} else {{
          totalLosses++;
          settlementActionNote = `Monday open momentum continuation. Settled @ $${{exitPrice.toFixed(2)}} (Entry $${{pos.entry_price.toFixed(2)}}). Anchor divergence caused loss.`;
          auditVerdict = "AUDITED_LOSS_MOMENTUM_OVERRUN";
          rootCause = `Price moved adversely to $${{exitPrice.toFixed(2)}} vs entry $${{pos.entry_price.toFixed(2)}}. Anchor convergence failed.`;
          adaptation = `Cognitive Auditor adapted entry threshold (+0.25σ) for ${{pos.symbol}} to reduce adverse momentum exposure.`;
        }}
        ...
        // ── POST-SETTLEMENT RETROSPECTIVE STRESS TEST ──
        // Run a fresh scenario analysis at current prices after settlement.
        // This answers: "If we were to retake this trade right now, would it clear?"
        // Stored in stressTestMemory so the agent never forgets what happened.
        const postAuditStress = runStressTest(pos.symbol, pos.entry_price, pos.side, pos.collateral, "POST_AUDIT");
        if (!postAuditStress.error) {{
          storeStressTestResult(pos.symbol, postAuditStress);
        }}

        // Cognitive Self-Auditor entry (with stress retrospective attached)
        const newAudit = {{
          trade_id: newTradeId,
          symbol: pos.symbol,
          side: pos.side,
          ...
          stress_retrospective: postAuditStress.error ? null : postAuditStress
        }};
    ...
    function showToast(title, message, type = "info", duration = 4200) {{
      ...
      const toastIcons = {{
        success: `<i data-lucide="check-circle" style="width:16px;height:16px;stroke:currentColor;stroke-width:2;"></i>`,
        warning: `<i data-lucide="alert-triangle" style="width:16px;height:16px;stroke:currentColor;stroke-width:2;"></i>`,
        error:   `<i data-lucide="x-circle" style="width:16px;height:16px;stroke:currentColor;stroke-width:2;"></i>`,
        info:    `<i data-lucide="info" style="width:16px;height:16px;stroke:currentColor;stroke-width:2;"></i>`
      }};
      ...
      toast.innerHTML = `
        ...
        <button type="button" class="toast-close" title="Dismiss"><i data-lucide="x" style="width:14px;height:14px;stroke:currentColor;stroke-width:2;"></i></button>
      `;
      refreshIcons();
    ...
    function handleEnvModeChange(env) {{
      ...
      if (env === "live") {{
        ...
        const btnClear = document.getElementById("btnClearBitget");
        if (btnClear) btnClear.style.display = "inline-flex";

        // Pre-fill credentials from wallet store ONLY if saved and valid
        const d = ChronosWalletStore.getCurrentData();
        const savedGw = d && d.gateway ? d.gateway : null;
        const hasSavedLiveKey = !!(savedGw && savedGw.mode === "live" && savedGw.apiKey && savedGw.apiKey.length < 50);
        if (apiKeyInput) {{
          apiKeyInput.value = hasSavedLiveKey ? savedGw.apiKey : "";
        }}
        if (apiSecretInput) {{
          apiSecretInput.value = hasSavedLiveKey ? (savedGw.apiSecret || "") : "";
        }}
        if (passphraseInput) {{
          passphraseInput.value = hasSavedLiveKey ? (savedGw.passphrase || "") : "";
        }}
        ...
        // 3. PRESERVE PAPER STATE & SWITCH TO LIVE STATE (NEVER WIPE PAPER DATA!)
        if (d) {{
          if (d.openPositions && d.openPositions !== d.liveOpenPositions) {{
            d.paperOpenPositions = d.openPositions.slice();
          }}
          if (d.trades && d.trades !== d.liveTrades) {{
            d.paperTrades = d.trades.slice();
          }}
          d.liveOpenPositions = d.liveOpenPositions || [];
          d.liveTrades = d.liveTrades || [];
          d.openPositions = d.liveOpenPositions;
          d.trades = d.liveTrades;
          ChronosWalletStore.saveData(ChronosWalletStore.currentAddress, d);
          renderActivePositions();
          renderOverviewDynamic(d);
          ChronosWalletStore.syncActiveView();
        ...
        // RESTORE PAPER STATE (NEVER WIPE LIVE DATA EITHER!)
        const d = ChronosWalletStore.getCurrentData();
        if (d) {{
          if (d.openPositions && d.openPositions === d.liveOpenPositions) {{
            d.liveOpenPositions = d.openPositions.slice();
          }}
          if (d.trades && d.trades === d.liveTrades) {{
            d.liveTrades = d.trades.slice();
          }}
          d.paperOpenPositions = d.paperOpenPositions || [];
          d.paperTrades = d.paperTrades || [];
          d.openPositions = d.paperOpenPositions;
          d.trades = d.paperTrades;
          ChronosWalletStore.saveData(ChronosWalletStore.currentAddress, d);
        }}

        restorePaperTradingState();
        renderActivePositions();
        if (d) renderOverviewDynamic(d);
        ChronosWalletStore.syncActiveView();

        showToast(
          "Switched to Internal Vault Mode",
          "Paper trading sandbox active. Your paper positions and balance are preserved.",
          "info"
        );
      }}
    }}
    ...
    function loadBitgetAccount() {{
      ...
      // ONLY treat as configured if explicitly saved as live mode with a clean, legitimate key
      const hasRealKey = !!(savedGw && savedGw.mode === "live" && savedGw.apiKey && savedGw.apiKey.trim().length > 0 && savedGw.apiKey.length < 50);
      const keyVal = hasRealKey ? savedGw.apiKey.trim() : "";

      const balanceDisplay = document.getElementById("settingsPaperBalanceDisplay");
      const headerChip = document.getElementById("headerBalanceChipSpan");
      const keyPreviewEl = document.getElementById("liveBitgetKeyPreview");
      const liveBalEl = document.getElementById("liveBitgetBalanceValue");
      const statusEl = document.getElementById("liveBitgetGatewayStatus");
      const walletLabel = document.getElementById("settingsWalletAddress");

      if (hasRealKey) {{
        const masked = keyVal.length > 8 ? keyVal.slice(0, 4) + "..." + keyVal.slice(-4) : keyVal;
        if (keyPreviewEl) keyPreviewEl.textContent = masked;
        if (walletLabel) {{
          walletLabel.innerHTML = `
            <div style="display: flex; gap: 0.5rem; align-items: center;">
              <span style="color: #065F46; font-weight: 700;">Key: ${{masked}}</span>
              <span class="badge-pill-light" style="font-size: 0.68rem;">UTA Live</span>
            </div>
          `;
        }}
        if (statusEl) {{
          statusEl.textContent = "Ready (Click Test Connection)";
          statusEl.style.color = "var(--color-green)";
        }}
        if (typeof d.liveBalance === "number") {{
          const balStr = `$${{d.liveBalance.toLocaleString('en-US', {{minimumFractionDigits: 2, maximumFractionDigits: 2}})}} USDT`;
          if (balanceDisplay) balanceDisplay.textContent = balStr;
          if (headerChip) headerChip.textContent = `$${{d.liveBalance.toLocaleString('en-US', {{minimumFractionDigits: 2, maximumFractionDigits: 2}})}}`;
          if (liveBalEl) liveBalEl.textContent = balStr;
        }} else {{
          if (balanceDisplay) balanceDisplay.textContent = "— USDT";
          if (headerChip) headerChip.textContent = "—";
          if (liveBalEl) liveBalEl.textContent = "— USDT";
        }}
      }} else {{
        if (keyPreviewEl) keyPreviewEl.textContent = "No Key Connected";
        if (walletLabel) {{
          walletLabel.innerHTML = `
            <div style="display: flex; gap: 0.5rem; align-items: center;">
              <span style="color: #9CA3AF; font-weight: 700;">Awaiting API Key</span>
              <span class="badge-pill-light" style="font-size: 0.68rem;">Not Configured</span>
            </div>
          `;
        }}
        if (balanceDisplay) balanceDisplay.textContent = "— USDT";
      ...
      // If user has saved credentials, trigger a real live balance sync
      if (hasRealKey && savedGw.apiSecret && savedGw.passphrase) {{
        syncBackendBalance();
      }}
    }}

    function clearBitgetCredentials() {{
      const d = ChronosWalletStore.getCurrentData();
      if (d) {{
        d.gateway = {{ mode: "paper", apiKey: "", apiSecret: "", passphrase: "" }};
        d.liveBalance = null;
        ChronosWalletStore.setCurrentData(d);
      }}
      const apiKeyInput = document.getElementById("settingsApiKey");
      ...
      handleEnvModeChange("paper");
      showToast("Credentials Cleared", "Bitget API credentials removed from your browser. Switched to Paper Simulation Vault.", "info");
    }}
    window.clearBitgetCredentials = clearBitgetCredentials;
    ...
    // ── Global icon helper ────────────────────────────────────────────────────
    // Returns an inline Lucide icon element string for use in innerHTML.
    // After any innerHTML update call window.refreshIcons() to activate new icons.
    function icon(name, {{ size = 14, color = "currentColor", strokeWidth = 1.75, style = "" }} = {{}}) {{
      return `<i data-lucide="${{name}}" style="width:${{size}}px;height:${{size}}px;stroke:${{color}};stroke-width:${{strokeWidth}};display:inline-block;vertical-align:middle;flex-shrink:0;${{style}}"></i>`;
    }}
    window.icon = icon;

    // Re-activates any newly injected data-lucide elements after innerHTML writes
    ...
    function initApp() {{
      // Activate all static Lucide icons on first paint
