...
<body>
 ...
 renderOverviewDynamic(ChronosWalletStore.getCurrentData());
    ...
    function loadBitgetAccount() {{
      ...
      const keyVal = (savedGw && savedGw.apiKey) ? savedGw.apiKey : (keyInput ? keyInput.value.trim() : "");
      ...
      if (keyVal) {{
        ...
        if (statusEl) {{
          statusEl.textContent = "Ready (Click Test Handshake)";
          statusEl.style.color = "var(--color-green)";
        }}
      }} else {{
        if (keyPreviewEl) keyPreviewEl.textContent = "Enter API Key Below";
        if (walletLabel) {{
          walletLabel.innerHTML = `
            <div style="display: flex; gap: 0.5rem; align-items: center;">
      ...
      // If user has saved credentials, trigger a real live balance sync immediately
      if (savedGw && savedGw.apiKey && savedGw.apiSecret && savedGw.passphrase) {{
        syncBackendBalance();
      }}
    }}
